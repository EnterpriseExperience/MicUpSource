...
if getgenv().Easies_Configuration["Default_Animation_Package_System"] == true then
    ...
    local function onCharacterAdded(Character)
        ...
        print("[Animation_Packages_System]: Character Added: " .. LocalPlayer.Name)
        wait(0.1)
        local Humanoid = Character:WaitForChild("Humanoid", 5)
        if not Humanoid then
            return warn("Humanoid not found, unable to apply animations.")
        end
    
        print("[Animation_Packages_System]: Found Character and Humanoid!")
        wait()
        Humanoid.Died:Connect(function()
            print("[Animation_Packages_System]: Player has died. Waiting for respawn...")
        end)
        wait(0.5)
        print("[Animation_Packages_System]: Connecting back to Animation Packages...")
        connect_to_animations()
        print("[Animation_Packages_System]: Connected to Animation Packages.")
    end
