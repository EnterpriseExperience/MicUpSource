...
--]]
    ...
    Callback = function(KeyForTripping)
        Trip_Settings.Keybind_Trip = KeyForTripping
    end,})

    getgenv().FakeOut_Keybind = Tab15:CreateKeybind({
    Name = "Fake Out Keybind",
    CurrentKeybind = tostring(Trip_Settings.Keybind_FakeOut),
    HoldToInteract = false,
    Flag = "FakingOutKeybind",
    Callback = function(setKeyForFakeOut)
        Trip_Settings.Keybind_FakeOut = setKeyForFakeOut
    end,})

    if getgenv().Trip_Enabled or getgenv().Trip_Enabled == true then
        ...
        if getgenv().Trip_Connection then
            ...
            getgenv().notify("Passed", "Trip_Connection is not currently enabled.", 5)
        end
    else
        getgenv().notify("Okay", "Trip is not currently enabled - passed", 5)
    end
    ...
    Callback = function(isTripOn)
        if isTripOn then
            ...
            getgenv().Trip_Enabled = false
            wait(0.3)
            getgenv().notify("Success", "Successfully disabled Trip Connection.")
        end
    end,})
