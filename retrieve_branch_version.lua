    ...
    local low_level_executor_result = low_level_executor() -- this function can be used as follows:
    -- if low_level_executor_result == true then
    -- print("This executor can run what ever function, put something else here obviously.")
    -- elseif low_level_executor_result == false then
    -- print("This executor is a low level executor and most likely has warned that in the console")
    -- end

    local function beta_tester(player)
        -- basically grabs everyone out of the table if you added them above. --
    ...
    -- oh? what's that? 67? 67 GUYS, 67! (sybau 🙏) --
    print("6")
    wait()
    print("7")
    wait()
    local function make_title(player, text, color, transparency)
        ...
        local function applyToCharacter(character)
            task.wait(0.5)
            -- funny enough, i will be the first to admit, the camel casing + the random returns at the end on the lines a little below here ARE infact ChatGPT --
            -- i was feeling terrible the day i originally made this system, and I certainly wasn't gonna do it myself then, and it works now, so. --
            if color == Color3.fromRGB(255, 255, 255) then
                ...
                textLabel.Size = UDim2.new(1, -10, 1, -10) -- as well as this, and the BillboardGui's size to, but I wouldn't quite recommend that.
                textLabel.Position = UDim2.new(0, 5, 0, 5)
                textLabel.BackgroundTransparency = 1
                textLabel.TextScaled = true
                textLabel.Font = Enum.Font.GothamBold
                textLabel.TextColor3 = Color3.fromRGB(0, 0, 0) -- This you infact CAN change though, if you want a different text color, this just seemed better.
                textLabel.TextStrokeTransparency = 0
                textLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
                -- keep this below, do NOT change it from here. --
                ...
                textLabel.TextStrokeTransparency = 0
                textLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
                textLabel.Text = text -- Don't change this shit either, it won't update, modify it below.
                textLabel.Parent = background
            end
        end
    ...
    getgenv().event_booth_Remote_Event_exit_save = function()
        local cache = getgenv().save_changes_booth_Remote_Event
        if cache and cache.Parent and cache:IsA("RemoteEvent") then
            return cache
        end

        for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
            if v:IsA("RemoteEvent") and v.Name:lower():find("event") and v.Name:lower():find("booth") and v.Name:lower():find("exit") then
                getgenv().save_changes_booth_Remote_Event = v
                return v
            end
        end
    ...
    wait(0.1)
    if not getgenv().save_changes_booth_Remote_Event then task.spawn(function() getgenv().event_booth_Remote_Event_exit_save() end) end

    getgenv().adjust_booth_Remote_Event_locator = function()
        local cache = getgenv().adjust_booth_settings_color_Remote_Event
        if cache and cache.Parent and cache:IsA("RemoteEvent") then
            return cache
        end

        for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
            if v:IsA("RemoteEvent") and v.Name:lower():find("event") and v.Name:lower():find("booth") and v.Name:lower():find("adjust") then
                getgenv().adjust_booth_settings_color_Remote_Event = v
                return v
            end
        end
    ...
    wait(0.1)
    if not getgenv().adjust_booth_settings_color_Remote_Event then task.spawn(function() getgenv().adjust_booth_Remote_Event_locator() end) end

    getgenv().delete_booth_RemoteEvent_locator = function()
        local cache = getgenv().delete_remove_booth_Remote_Event_found
        if cache and cache.Parent and cache:IsA("RemoteEvent") then
            return cache
        end

        for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
            if v:IsA("RemoteEvent") and v.Name:lower():find("event") and v.Name:lower():find("booth") and v.Name:lower():find("delete") then
                getgenv().delete_remove_booth_Remote_Event_found = v
                return v
            end
        end
    ...
    getgenv().get_booth = function()
        if getgenv().Current_Claimed_Booth and getgenv().Current_Claimed_Booth:IsA("Folder") then
            return getgenv().Current_Claimed_Booth
        else
            return getgenv().Current_Claimed_Booth
        end
    end

    getgenv().has_booth = function()
        if getgenv().Current_Claimed_Booth and getgenv().Current_Claimed_Booth:IsA("Folder") and getgenv().Current_Claimed_Booth.Parent then
            return true
        else
            return false
        end
    ...
    getgenv().notify = getgenv().notify or function(title, content, duration)
    ...
    CurrentValue = getgenv().parts_rainbow_anti_void or false,
        ...
        if GetRainbowStateAntiVoid then
            ...
            local function cycleColor()
                if not getgenv().parts_rainbow_anti_void then
                    return getgenv().notify("Warning", "AntiVoid BasePlate rainbow loop disabled.", 6)
                end
    ...
    CurrentValue = getgenv().smooth_rainbow or false,
    ...
    CurrentValue = getgenv().no_baseplate_collision or false,
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        CurrentValue = getgenv().toggling_all_booth_collisions or false,
        Flag = "NoClippingBooths",
        Callback = function(noclip_the_booths)
            getgenv().toggling_all_booth_collisions = noclip_the_booths

            if noclip_the_booths then
                for _, v in ipairs(getgenv().booth_folder_found_mic_up:GetDescendants()) do
                    if v:IsA("BasePart") then
                        v.CanCollide = true
                    end
                end
        ...
        if getgenv().booth_folder_found_mic_up and getgenv().booth_folder_found_mic_up.Parent and getgenv().booth_folder_found_mic_up:IsDescendantOf(workspace) and getgenv().booth_folder_found_mic_up:IsA("Folder") then
            for _, v in ipairs(getgenv().booth_folder_found_mic_up:GetDescendants()) do
                if v:IsA("BasePart") then
                    v.CanCollide = true
                end
        ...
        Callback = function(transparency_value_booths)
            for _, v in ipairs(getgenv().booth_folder_found_mic_up:GetDescendants()) do
                if v:IsA("BasePart") and not v.Name:lower():find("light") then
                    local accessory = v.Parent
                    if accessory.ClassName == "Accessory" and accessory.Parent.Name:lower():find("npc") then
            ...
            if transparency_value_booths == 1 then
                for _, v in ipairs(getgenv().booth_folder_found_mic_up:GetDescendants()) do
                    if v:IsA("Folder") then
                        local prox_prompt_part_parent = v.Name:lower():find("claim")

                        if prox_prompt_part_parent and prox_prompt_part_parent:FindFirstChildOfClass("ProximityPrompt") then
                            prox_prompt_part_parent:FindFirstChildOfClass("ProximityPrompt").Enabled = true
                        end
                    end
                end
            else
        ...
        if getgenv().Workspace:FindFirstChild("map") then
            if getgenv().booth_folder_found_mic_up then
                for _, v in ipairs(getgenv().booth_folder_found_mic_up:GetDescendants()) do
        ...
        Name = "Copy Player Avatar (broken)",
    ...
    if not getgenv().CameraZoomDefaults then
        getgenv().CameraZoomDefaults = {
            Min = getgenv().LocalPlayer.CameraMinZoomDistance,
            Max = getgenv().LocalPlayer.CameraMaxZoomDistance,
        }
    end
    task.wait(0.2)
    getgenv().CameraMaxZoomDistance = Tab5:CreateSlider({
    Name = "Max Zoom Distance (Default): "..tostring(getgenv().CameraZoomDefaults.Max),
    Range = {25, 100000},
    Increment = 1,
    Suffix = "",
    CurrentValue = tonumber(getgenv().LocalPlayer.CameraMaxZoomDistance),
    ...
    Name = "Min Zoom Distance (Default): "..tostring(getgenv().CameraZoomDefaults.Min),
    Range = {0.5, 1000},
    Increment = 0.5,
    Suffix = "",
    CurrentValue = tonumber(getgenv().LocalPlayer.CameraMinZoomDistance),
    ...
    Callback = function(anims_spoofed)
        ...
        if anims_spoofed then
            ...
            getgenv().custom_anim_conn = run_service.Heartbeat:Connect(function()
                if not getgenv().are_animations_spoofed then return end
                getgenv().StarterPlayer.AllowCustomAnimations = true
            end)
            ...
            if getgenv().custom_anim_conn then
                getgenv().custom_anim_conn:Disconnect()
                getgenv().custom_anim_conn = nil
            end
    ...
    Callback = function(toggle_hd_fly)
        if toggle_hd_fly then
            ...
            if HDAdmin_Found == false and xeno_level_executor == true then
                ...
                local Humanoid = getgenv().Humanoid or getgenv().Character and getgenv().Humanoid or get_human(LocalPlayer, 10)
    ...
    Callback = function(enable_anti_fling_script)
        if enable_anti_fling_script then
            ...
            local function get_hrp()
                ...
                return getgenv().HumanoidRootPart or char and char:FindFirstChild("HumanoidRootPart") or getgenv().Humanoid
            end
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Callback = function(apply_to_respawn_anims)
            if apply_to_respawn_anims then
                ...
                getgenv().ownerAnimsEnabled = false
                if getgenv().Humanoid and getgenv().Character:FindFirstChild("HumanoidRootPart") then
                    getgenv().Humanoid.WalkSpeed = 0
                    getgenv().getRoot(getgenv().Character).Anchored = false
                    wait(1.5)
                    local humanoid = getgenv().Humanoid
    ...
    if game.PlaceId == 17274762379 then
        ...
        Callback = function(anim_play)
            ...
            if not animationHandlerEvent or not animationHandlerEvent:IsA("RemoteEvent") then
                return getgenv().notify("Error", "animationHandlerEvent RemoteEvent does not exist or is not a RemoteEvent!", 10)
            end 
        ...
        Callback = function()
            ...
            if Codes and Codes.Parent and Codes:IsA("RemoteFunction") then
        ...
        Callback = function(player_entered)
            ...
            if not found_this_plr then
                return getgenv().notify("Error", "Player was not found or does not exist.", 6)
            end
        ...
        if require and typeof(require) == "function" then
            ...
            Callback = function(text_to_display)
                ...
                getgenv().notify("Warning", "If it gives you an error it means you can't do it.", 6)
        ...
        Callback = function(player_focus_bruh)
            ...
            local function addPlayerToGlobalTable(player)
                if next(getgenv().SpamWhitelistTable) then
                    return getgenv().notify("Warning", "Only one player can be added at a time!", 5)
                else
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Callback = function(getThisPlayer)
            ...
            if not bruhUser then
                return getgenv().notify("Error", "Player was not found.", 6)
            end
        ...
        Callback = function(RemoveBoothWhitelist)
            ...
            local function removePlayerFromBoothTable(player)
                if getgenv().boothWhitelistingPlayer[player.Name] then
                    ...
                    return getgenv().notify("Error", tostring(player.Name)..", was not found in Booth Whitelist!", 5)
                end
            end
    ...
    Callback = function(removeThatUserOverHere)
        ...
        local function removePlayerFromScriptWhitelistTable(player)
            if getgenv().script_whitelist_player[player.Name] then
                ...
                return getgenv().notify("Error", tostring(player.Name)..", was not found in Script Whitelist!", 5)
            end
        end
    ...
    if game.PlaceId == 97399198116506 then
        ...
        Callback = function()
            if MainMenu and MainMenu:IsA("RemoteFunction") then
            ...
            local function disableConnectionsForPropertiesAndEvents(character)
                ...
                for _, propData in ipairs(properties) do
                    ...
                    if object and object:GetPropertyChangedSignal(property) then
                        ...
                        getgenv().notify("Error", "Could not find signal: "..tostring(property)..", for: "..tostring(object.Name), 5)
                    end
                end
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Name = "Rainbow Booth (FE)",
        CurrentValue = getgenv().rgb_booth_enabled or false,
        Flag = "BoothBeingRainbowClientSide",
        Callback = function(enabled)
            getgenv().rgb_booth_enabled = enabled

            if enabled then
                getgenv().rgb_booth_speed = 0.7
                getgenv().FlamesLibrary.spawn("booth_rgb_loop", "spawn", function()
                    local hue = 0

                    while getgenv().FlamesLibrary.is_alive("booth_rgb_loop") do
                        hue = (hue + getgenv().rgb_booth_speed) % 1

                        getgenv().save_changes_booth_Remote_Event:FireServer({
                            texture_preset = "fabric",
                            title_text = "",
                            texture_values = {
                                ["02"] = {hue_number = hue,value_number = 0.4901960790157318,saturation_number = 1},
                                title = {hue_number = hue,value_number = 0.9803921580314636,saturation_number = 1},
                                ["01"] = {hue_number = hue,value_number = 0,saturation_number = 1}
                            }
                        })
                ...
                getgenv().FlamesLibrary.disconnect("booth_rgb_loop")
            end
        end,})
    ...
    Callback = function(doRemoveAnyLaserEyes)
        if doRemoveAnyLaserEyes then
            ...
            wait(0.6)
                for _, player in pairs(Players:GetPlayers()) do
                    if player ~= getgenv().LocalPlayer and player and player.Character then
    ...
    Callback = function()
        local Character = getgenv().Character or LocalPlayer.Character or get_char(LocalPlayer, 10)
        local Humanoid = getgenv().Humanoid or getgenv().Humanoid or get_human(LocalPlayer, 15)
    ...
    Callback = function(getLoopCopyAnim)
        if getLoopCopyAnim then
            ...
            local Humanoid = getgenv().Humanoid or getgenv().Humanoid or get_human(LocalPlayer, 10)
            local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character:FindFirstChild("HumanoidRootPart") or get_root(LocalPlayer, 15)
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        if getgenv().Workspace:FindFirstChild("map") then
            local Map_Folder = getgenv().Workspace:FindFirstChild("map")
            local Important_Folder = Map_Folder and Map_Folder:FindFirstChild("Important")
            local Baseplate_Folder = Important_Folder and Important_Folder:FindFirstChild("Baseplate")
            local Baseplate_Part = Baseplate_Folder and Baseplate_Folder:FindFirstChild("Baseplate")
    ...
    getgenv().Reset_Map_Color = Tab18:CreateButton({
    Name = "Reset Map Colors",
    Callback = function()
    ...
    getgenv().find_mic_up_baseplate = function()
        ...
        if Map then
            ...
            if Important then
                local Baseplate_Folder = Important:FindFirstChild("Baseplate")
                if not Baseplate_Folder then return end
    ...
    Callback = function()
        ...
        local Humanoid = getgenv().Character and getgenv().Humanoid or get_human(LocalPlayer, 10)
    ...
    getgenv().fuck = function()
        ...
        local hum = getgenv().Humanoid or getgenv().Character:WaitForChild("Humanoid")
    ...
    Callback = function()
        ...
        getgenv().onRenderStep = function()
            if getgenv().Character:FindFirstChild("HumanoidRootPart") and getgenv().Humanoid then
                local character = getgenv().Character
                local humanoidRootPart = getgenv().Character:FindFirstChild("HumanoidRootPart")
                local humanoid = getgenv().Humanoid
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Callback = function(specBooth)
            local plr_booth = has_booth()
            if not plr_booth then
                return getgenv().notify("Error", "You do not have a booth, claim one!", 5)
            end
            local get_plr_booth = get_booth()
            if not get_plr_booth then
                return getgenv().notify("Error", "You do not have a booth, claim one!", 5)
            end

            local spectate_part

            for _, v in ipairS(get_plr_booth:GetDescendants()) do
                if v:IsA("BasePart") and v.Name:lower():find("claim") then
                    spectate_part = v
                end
            end
            wait(0.2)
            if specBooth then
                if spectate_part then
                    getgenv().viewing_booth = true
                    getgenv().Camera.CameraSubject = spectate_part
                else
                    getgenv().viewing_booth = true
                    getgenv().Camera.CameraSubject = get_plr_booth and get_plr_booth:FindFirstChildOfClass("Part")
                end
            else
                getgenv().viewing_booth = false
                getgenv().Camera.CameraSubject = getgenv().Character or getgenv().Humanoid or getgenv().Humanoid
            end
        end,})
        wait(0.1)
        if getgenv().viewing_booth then
            getgenv().Camera.CameraSubject = getgenv().Humanoid or getgenv().Character and getgenv().Humanoid
        end
        ...
        Callback = function()
            ...
            local booth_teleport_part

            for _, v in ipairS(get_plr_booth:GetDescendants()) do
                if v:IsA("BasePart") and v.Name:lower():find("claim") then
                    booth_teleport_part = v
                end
            end

            pcall(function() getgenv().Character:PivotTo(booth_teleport_part:GetPivot()) end)
        end,})
    else
    ...
    Callback = function(do_plr_func)
        ...
        if spectate_char and spectate_char:FindFirstChild("Humanoid") and spectate_char:FindFirstChild("HumanoidRootPart") then
    ...
    elseif identifyexecutor then
        getgenv().notify("Failed!", "Your executor: "..tostring(executor_Name).." does not support 'setfpscap'", 5)
    else
        getgenv().notify("Failure!", "Your executor does is garbage.", 5)
    end

    if getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput") then
        function shut_down_mute_connections()
            if getgenv().Muted_Microphone_Connection_AudioDeviceInput then
                getgenv().Muted_Microphone_Connection_AudioDeviceInput:Disconnect()
                getgenv().Muted_Microphone_Connection_AudioDeviceInput = nil
            end
        end

        getgenv().MutingMyself = Tab21:CreateToggle({
        Name = "Mute Yourself (FE)",
        CurrentValue = false,
        ...
        Callback = function(muteYou)
            if muteYou then
                ...
                local input = getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput")
                if not input then
                    shut_down_mute_connections()
                    getgenv().MutingMyself:Set(false)
                    return getgenv().notify("Failure:", "AudioDeviceInput does not exist in LocalPlayer!", 5)
                end

                if not getgenv().Muted_Microphone_Connection_AudioDeviceInput then
                    getgenv().Muted_Microphone_Connection_AudioDeviceInput = getgenv().RunService.Heartbeat:Connect(function()
                        local device = getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput")
                        if device then
                            device.Muted = true
                        end
                    end)
        ...
        if Enum.KeyCode[keybindString] then
            ...
            getgenv().notify("Failure:", "Invalid keybind string: "..tostring(keybindString), 5)
            getgenv().notify("Heads Up:", "Alternating mute keybind to: RightControl.", 5)
        ...
        if cloneref then
            getgenv().notify("Hang On:", "Using cloneref for Muted keybind.", 5)
            Workspace = cloneref(game:GetService("Workspace"))
            Players = cloneref(game:GetService("Players"))
            ReplicatedStorage = cloneref(game:GetService("ReplicatedStorage"))
            UserInputService = cloneref(game:GetService("UserInputService"))
        else
            getgenv().notify("Error:", "'cloneref' is unsupported, using normal method to grab services.", 5)
        ...
        if getgenv().already_loaded_mute_keybind then
            getgenv().notify("Skipping", "Already initialized mute keybind.", 5)
        else
            ...
            getgenv().UserInputService.InputBegan:Connect(function(input, gameProcessed)
                if gameProcessed then return end
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Callback = function()
            ...
            local Delete_Inv_RemoteEvent = Replicated_Storage:FindFirstChild("ToolDelete")
            if not Delete_Inv_RemoteEvent then return getgenv().notify("Failure:", "ToolDelete RemoteEvent doesn't exist in ReplicatedStorage!", 5) end
            
            if Delete_Inv_RemoteEvent then
        ...
        function Tools_Grab_Func()
            ...
            local success, hasPass = pcall(function()
                return MarketplaceService:PlayerOwnsAsset(getgenv().LocalPlayer, gamepassId)
            end)
            ...
            local Hub_Frame = Menu_GUI:FindFirstChild("Hub")
            local Tool_Frame = Hub_Frame:FindFirstChild("Tool")
            local Left_Frame = Tool_Frame:FindFirstChild("Left")
            local All_Tools_Scrolling_Frame = Left_Frame:FindFirstChild("ScrollingFrame")
            getgenv().All_MIC_UP_Tools = getgenv().All_MIC_UP_Tools or {}

            for _, v in ipairs(All_Tools_Scrolling_Frame:GetChildren()) do
                if v:IsA("Frame") then
                    table.insert(getgenv().All_MIC_UP_Tools, v.Name)
                end
            end
            ...
            local ToolEvent = ReplicatedStorage and ReplicatedStorage:FindFirstChild("Tool") or ReplicatedStorage:WaitForChild("Tool", 3)
            ...
            for _, tool_name in pairs(getgenv().All_MIC_UP_Tools) do
                PickUp_Tool(tool_name)
                task.wait(0.7)
            end
        end
        ...
        Callback = function(grabFreeTools)
            if grabFreeTools then
                ...
                local Delete_Inv_Event = Rep_Storage:WaitForChild("ToolDelete") or Rep_Storage:FindFirstChild("ToolDelete")
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Callback = function()
            ...
            if getgenv().Humanoid.Sit then
                getgenv().Humanoid:ChangeState(3)
                task.wait(.2)
                local Booth_Folder = getgenv().booth_folder_found_mic_up
        ...
        Callback = function()
            ...
            if getgenv().Humanoid.Sit then
                getgenv().Humanoid:ChangeState(3)
                task.wait(.2)
                local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character and getgenv().Character:FindFirstChild("HumanoidRootPart")
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 or game.PlaceId == 80080558412215 then
        ...
        CurrentValue = getgenv().doAllClicks or false,
    ...
    Callback = function()
        if getgenv().IY_LOADED then
            return getgenv().notify("Warning", "Infinite Yield cannot be ran while Infinite Premium is running.", 6)
        end
    ...
    Callback = function()
        if getgenv().IY_LOADED then
            return getgenv().notify("Warning", "Infinite Yield is already loaded.", 6)
        end
        if getgenv().GET_LOADED_IY then
            return getgenv().notify("Warning", "Infinite Premium cannot be ran while Infinite Yield is running.", 6)
        end
    ...
    local Old_WalkSpeed = getgenv().Humanoid.WalkSpeed
    local Old_HipHeight = getgenv().Humanoid.HipHeight
    local Old_Gravity = getgenv().Workspace.Gravity

    if getgenv().Humanoid.UseJumpPower or getgenv().Humanoid.UseJumpPower == true then
        Old_JumpPower_JumpHeight = getgenv().Humanoid.JumpPower
    else
        Old_JumpPower_JumpHeight = getgenv().Humanoid.JumpHeight
    end
    wait(0.5)
    ...
    Callback = function()
        getgenv().Humanoid.WalkSpeed = Old_WalkSpeed
    ...
    Callback = function()
        if getgenv().Humanoid.UseJumpPower then
            getgenv().Humanoid.JumpPower = Old_JumpPower_JumpHeight
        else
            getgenv().Humanoid.JumpHeight = Old_JumpPower_JumpHeight
        end
    ...
    Callback = function(hipValue)
        pcall(function() getgenv().Humanoid.HipHeight = hipValue end)
    end,})
    ...
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        Name = "Anti Bang/TP (Loop Method, Broken?)",
        CurrentValue = getgenv().doTeleport or false,
    ...
    CurrentValue = getgenv().loopTPToVoid or false,
    Flag = "AntiTPMethod4",
    ...
    Increment = 1,
    Suffix = "",
    CurrentValue = getgenv().Lighting.ClockTime or 14,
    ...
    CurrentValue = getgenv().Locking_ClockTime_Toggle_Enabled or false,
    ...
    getgenv().use_new_meterials = function(toggle)
        ...
        elseif toggle == false then
            ...
            getgenv().notify("Error", "You provided an unknown or incorrect argument.", 6)
            return 
        end
    end
    ...
    Name = "Mute All",
    CurrentValue = getgenv().LmaoGetMuted or false,
    Flag = "MutingEveryone",
    Callback = function(TheMuteAll)
        if TheMuteAll then
            ...
            if not enabled_vc or enabled_vc == false then
                return getgenv().notify("Error", "Your account does not have VoiceChat!", 5)
            end
    ...
    getgenv().MutePlayer = function(user)
        if not user then
            return getgenv().notify("Warning", "User not found", 5)
        end
        ...
        if getgenv().MutedPlayers[user.UserId] then
            return getgenv().notify("Warning", "User is already muted", 5)
        end
    ...
    getgenv().UnmutePlayer = function(user)
        ...
        if not getgenv().MutedPlayers[user.UserId] then
            return getgenv().notify("Error", tostring(user).." is not muted!", 5)
        end
    ...
    Callback = function(getAUser)
        ...
        if not derUser then
            return getgenv().notify("Error", "User was not found.", 5)
        end
    ...
    Name = "Reset",
    Callback = function()
        local Humanoid = getgenv().Humanoid or getgenv().Character and getgenv().Character:FindFirstChildOfClass("Humanoid") or get_human(LocalPlayer, 10)
    ...
    Callback = function()
        ...
        if httprequest then
            ...
            if body and body.data then
                ...
                return getgenv().notify("Error", "Unable to retrieve any data from the current experience.", 5)
            end
        else
        ...
        if #getgenv().servers > 0 then
            ...
            return getgenv().notify("Error", "Couldn't find a server [failed].", 5)
        end
    end,})
    ...
    if getgenv().Rayfield then
        ...
        Callback = function(UIColor)
            if not getgenv().Rayfield or getgenv().Rayfield == nil then
                getgenv().notify("Warning", "If this causes errors, it means Rayfield UI or it's global value is actually missing.", 8)
                getgenv().notify("Error", "You are most likely using the regular UI, don't worry.", 8)
            end
            ...
            local function changeColors(object, frameColor)
                if object and object:IsA("ScreenGui") then
                    ...
                    getgenv().notify("Error", "Rayfield ScreenGui not found!", 8)
                end
            end
    ...
    Callback = function()
        getgenv().notify("Info", "Loading update: "..tostring(Version), 5)
        Module:Patch_Update()
    end,})
    wait(0.1)
    ...
    Callback = function(LolPUser)
        ...
        if not ok or not enabled then
            --getgenv().notify("Error", "Your account does not have VoiceChat!", 5)
            return 
        end
    ...
    Callback = function(speedCoil)
        if speedCoil then
            ...
            if getgenv().Character then
                local humanoid = getgenv().Humanoid or getgenv().Humanoid
                if humanoid then
                    humanoid.WalkSpeed = getgenv().walkSpeed
                end
            end
    ...
    wait(0.1)
    -- [[ there's too many of these for me to work on it right now, I have a method to make these run a lot smoother and easier, coming soon. ]] --
    create_Button(CatWalkGlamAnim, "'Catwalk Glam' Animation Package", function()
        ...
        local player = getgenv().LocalPlayer
        local character = getgenv().Character
        local Animate = get_animate_localscript() or character and character:FindFirstChild("Animate")
        local Old_WS = getgenv().Humanoid.WalkSpeed

        if getgenv().Humanoid.RigType == Enum.HumanoidRigType.R6 then
            return getgenv().notify("Error", "You must be in R15 to use animation packages.", 6)
        end

        if not Animate then return getgenv().notify("Error", "Animate LocalScript does not exist in Character!", 5) end
    ...
    create_Button(ZombieFEAnim, "(FE) Zombie Animation Package", function()
        ...
        getgenv().Humanoid:ChangeState(3) -- I change my mind, some of them do jump (edited: 9/6/2025 : 9:53 AM)
        wait(0.3)
        Animate.Disabled = false
        wait(apply_anims_delay)
        pcall(function() getgenv().Humanoid.WalkSpeed = 16 end)
    end)
    ...
    Callback = function()
        if not writefile then return getgenv().notify("Error", "'writefile' is not supported in your exploit.", 7) end
            ...
            if isfile and isfile("EmoteConfig.json") then
                ...
                for keyName, emote in pairs(data) do
                    ...
                    if keyEnum then
                        ...
                        getgenv().notify("Success", "Key: " .. keyName .. " -> Emote: " .. emote, 5)
                    else
                ...
                getgenv().notify("Error", "Emote Configuration was not found or 'isfile' is not supported.", 7)
            end
        end
        wait()
    ...
    Callback = function()
        function erase_config()
            if isfile and isfile("EmoteConfig.json") then
                ...
                getgenv().notify("Error", "No configuration file found or 'isfile' is unsupported in your exploit!", 10)
            end
        end     
    ...
    Name = "Trip Keybind",
    -- I must have been smoking something or some shit when writing this code, what the hell is "Euro"? and I must have REALLY dug into Roblox's keycode API, because what the hell is ButtonStart??? 😭😭
    ...
    Options = {"Q","E","R","T","Y","U","I","O","P","F","G","H","J","K","L","Z","X","C","V","B","N","M",
               ...
               "ButtonA","ButtonY","ButtonB","ButtonR1","ButtonL1","ButtonL2","ButtonR2","ButtonL3",
               "ButtonStart","ButtonSelect","DPadLeft","DPadRight","DPadUp","DPadDown","Thumbstick1", "Thumbstick2"},
    CurrentOption = tostring(Trip_Settings.Keybind_FakeOut) or "R",
    MultipleOptions = false,
    ...
    getgenv().Trip_Script = Tab15:CreateKeybind({
    Name = "Trip",
    CurrentKeybind = "T",
    HoldToInteract = false,
    ...
    Callback = function(isTripOn)
        if isTripOn then
            ...
            local Character = getgenv().Character or LocalPlayer.Character or get_char(LocalPlayer, 10)
            local Humanoid = Character:FindFirstChildOfClass("Humanoid")
            local RootPart = getgenv().HumanoidRootPart or Character and Character:FindFirstChild("HumanoidRootPart") or get_root(LocalPlayer, 10)
    ...
    CurrentValue = getgenv().turned_off_booth_GUI or false,
    ...
    getgenv().EmoteSystemEnabled = getgenv().EmoteSystemEnabled or false
    getgenv().ToggleEmoteKeybinds = Tab20:CreateToggle({
    Name = "Emote Speed Keybinds",
    CurrentValue = getgenv().EmoteSystemEnabled or false,
    ...
    Name = "Fake Out",
    CurrentValue = getgenv().FakeOut_Enabled or false,
    ...
    Callback = function()
        ...
        if PlayerGui:FindFirstChild("Script_Clock_Time_Day_Night") then
            ...
            return getgenv().notify("Error", "Script Clock Time GUI does not exist.", 5)
        end
    end,})
    wait()
