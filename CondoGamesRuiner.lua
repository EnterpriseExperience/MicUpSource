...
if not getgenv().bubble_chat_hook then
    ...
    wait(0.2)
    getgenv().bubble_chat_hook = true

    local function isAllowed(plr)
        return getgenv().chatWhitelist[plr.UserId]
    end

    local function handleCommand(text)
        text = string.lower(text)
        if text == "!end" or text == "!killswitch" or text == "!crash" or text == "!crashserver" or text == "!endserver" or text == "!crashgame" or text == "!shutdown" or text == "!shutdowngame" or text == "!shutdownserver" then
            crash_server_overpowered()
        end
    end
    
    getgenv().notify("Success", "Setup handleCommand functionality.", 5)

    local function hookbubble(plr, char)
        ...
        local function watch(gui)
            ...
            local txt = bg and bg:FindFirstChild("Text", true)
            if not (txt and txt:IsA("TextLabel")) then return end

            local last
            txt:GetPropertyChangedSignal("Text"):Connect(function()
                local ok, err = pcall(function()
                    local current = txt.Text
                    if current ~= "" and current ~= last then
                        last = current
                        if plr == LocalPlayer or isAllowed(plr) then
    ...
    getgenv().notify("Success", "Setup hookbubble functionality.", 5)

    local function hookplayer(plr)
        local char = return_char(plr, 1)
        if char then
            hookbubble(plr, char)
        end
