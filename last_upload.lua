...
--]]
    ...
    task.wait(0.1)
    local function connect_to_animations()
        local Character = LocalPlayer.Character
        if not Character then 
            return warn("Character is missing.") 
        end
        ...
        local config = getgenv().Easies_Configuration or {}
        ...
        if change_idle_anim and anims.Idle then change_idle_anim(unpack(anims.Idle)) end
        if change_walk_anim and anims.Walk then change_walk_anim(anims.Walk) end
        if change_run_anim and anims.Run then change_run_anim(anims.Run) end
        if change_fall_anim and anims.Fall then change_fall_anim(anims.Fall) end
        if change_jump_anim and anims.Jump then change_jump_anim(anims.Jump) end
        if change_climb_anim and anims.Climb then change_climb_anim(anims.Climb) end
    
        print("Applied animation package:", packageName)
    end

    local function monitor_animate_script()
    ...
    connect_anims()
    task.spawn(monitor_animate_script)
