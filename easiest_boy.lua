...
--]]
    ...
    local function replaceAnimateScript()
        ...
        if not Character then 
            return warn("Character is missing.") 
        end
    
        local existingAnimate = Character:FindFirstChild("Animate")
        if existingAnimate then
            existingAnimate:Destroy()
            print("Removed default Animate script.")
        end
    
        local animateClone = script:FindFirstChild("AnimateClone")
        if animateClone then
            local newAnimate = animateClone:Clone()
            newAnimate.Parent = Character
            print("Replaced Animate script after respawn.")
        else
            warn("No AnimateClone found in script!")
        end
    end
    ...
    function connect_to_animations()
        ...
        if not animateScript then
            return warn("Animate script not found in Character.")
        end
        ...
        if not anims then
            return warn("Unknown animation package:", packageName)
        end
    ...
    function monitor_animate_script()
        while true do
            wait(1)
            local Character = LocalPlayer.Character
            if Character then
                local animateScript = Character:FindFirstChild("Animate")
                if not animateScript then
                   print("Animate script missing! Replacing it...")
                   replaceAnimateScript()
    ...
    function connect_anims()
        LocalPlayer.CharacterAdded:Connect(function(newCharacter)
            ...
            if oldAnimate then
                oldAnimate:Destroy()
                print("Destroyed default Animate script")
            end
        ...
        spawn(monitor_animate_script)
    end
    
    connect_anims()
    getgenv().animation_package_system = true
    print("Animation package system initialized and fixed for executor.")
