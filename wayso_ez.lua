...
--]]
    ...
    Callback = function(Bind_Freeze)
        if not Bind_Freeze or not Enum.KeyCode[Bind_Freeze] then 
        ...
        getgenv().notify("Success:", "Freeze keybind updated to: " .. tostring(Bind_Freeze), 6)
    end,})
    ...
    Callback = function(Bind_Reverse)
        if not Bind_Reverse or not Enum.KeyCode[Bind_Reverse] then 
            return warn("Invalid keybind selection:", Bind_Reverse) 
        end
        local keyCode = Enum.KeyCode[Bind_Reverse]
        wait()
        Emote_Speed_Configuration[keyCode] = 0
        getgenv().notify("Success:", "Reverse keybind updated to: " .. tostring(Bind_Reverse), 6)
    end,})
