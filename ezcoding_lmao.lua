...
--]]
    ...
    wait()
    local UserInputService = game:GetService("UserInputService")
    local Reverse_Keybind = Enum.KeyCode.F
    local Freeze_Keybind = Enum.KeyCode.V
    ...
    getgenv().Emote_Freeze_Keybind = Tab15:CreateKeybind({
    Name = "Freeze Keybind",
    CurrentKeybind = "V",
    HoldToInteract = false,
    Flag = "Freezing_Keybind_Change",
    Callback = function(Bind_Freeze)
        print(tostring(Bind_Freeze))
        if not Bind_Freeze then
            return warn("Invalid keybind selection:", Bind_Freeze)
        end
        wait()
        Freeze_Keybind = Bind_Freeze
        Emote_Speed_Configuration[Freeze_Keybind] = 0
        getgenv().notify("Success:", "Freeze keybind updated to: " .. tostring(Freeze_Keybind), 6)
    end,})

    getgenv().Emote_Reverse_Keybind = Tab15:CreateKeybind({
    Name = "Reverse Keybind",
    CurrentKeybind = "F",
    HoldToInteract = false,
    Flag = "Reverse_Keybind_Change",
    Callback = function(Bind_Reverse)
        print(tostring(Bind_Reverse))
        if not Bind_Reverse then 
            return warn("Invalid keybind selection:", Bind_Reverse) 
        end
        wait()
        Reverse_Keybind = Bind_Reverse
        Emote_Speed_Configuration[Reverse_Keybind] = -1
        getgenv().notify("Success:", "Reverse keybind updated to: " .. tostring(Reverse_Keybind), 6)
    end,})
    ...
    local function other_Input_Connecting(input, gameProcessed)
        ...
        if input.KeyCode == Reverse_Keybind then
            speedToggle = (speedToggle == 1) and -1 or 1
            Emote_Speed_Configuration[Reverse_Keybind] = speedToggle
            getgenv().emoting_actions(speedToggle)
        elseif input.KeyCode == Freeze_Keybind then
            getgenv().emoting_actions(0)
        else
