    ...
    elseif (enabled_vc == false and (not game.PlaceId == 6884319169)) or (not game.PlaceId == 15546218972) then
        Notification.Notify("Welcome", tostring(game.Players.LocalPlayer.Name).." does not have Voice Chat.", "rbxassetid://93594537601787", {
            Duration = 5,
            Main = {
                Rounding = true,
            }
    ...
    local ChatChannel = TextChatService:WaitForChild("TextChannels"):FindFirstChild("RBXGeneral")
    local SECRET_CHAT_CODE = "- ? -"
    ...
    local activeClientsFolder = getOrCreateFolder("ActiveClients")
    
    local support_team = {"Ixgale7530", "starsorbitspace", "goldgoldgoldBlazn", "euigwerergre", "o7rssuk"}
    local admin_team = {"Ransacked007", "IIIIDOXII", "IIIIDOXIII"}
    getgenv().ActiveClients = getgenv().ActiveClients or {}
    
    local function is_support_team()
        for _, v in pairs(game.Players:GetPlayers()) do
            if table.find(support_team, v.Name) then
                return true
            end
        end
        return false
    ...
    local function is_admin_team()
        for _, v in pairs(game.Players:GetPlayers()) do
            if table.find(admin_team, v.Name) then
                return true
            end
        end
        return false
    ...
    wait()
    local function createTitle(Plr)
        if getgenv().ActiveClients[Plr.UserId] then return end
        getgenv().ActiveClients[Plr.UserId] = Plr.Name
        
        local playerData = activeClientsFolder:FindFirstChild(tostring(Plr.UserId))
        if not playerData then
            playerData = Instance.new("StringValue")
            playerData.Name = tostring(Plr.UserId)
            playerData.Value = Plr.Name
            playerData.Parent = activeClientsFolder
        end
        ...
        elseif Plr.Name == "adorxfleurys" or Plr.Name == "lucxd19K5" and character and not character:FindFirstChild("ZacksEasyBillboard") then
            ...
            textLabel.Parent = background
        elseif is_support_team() and character and character:FindFirstChild("ZacksEasyBillboard") then
            local billboardGui = Instance.new("BillboardGui")
            billboardGui.Name = "ZacksEasyBillboard"
            billboardGui.Size = UDim2.new(10, 0, 1.5, 0)
            billboardGui.MaxDistance = math.huge
            ...
            textLabel.Parent = background
        elseif is_admin_team() and character and character:FindFirstChild("ZacksEasyBillboard") then
            local billboardGui = Instance.new("BillboardGui")
            billboardGui.Name = "ZacksEasyBillboard"
            billboardGui.Size = UDim2.new(10, 0, 1.5, 0)
            billboardGui.MaxDistance = math.huge
    ...
    Players.PlayerAdded:Connect(function(Plr)
        task.wait(1)
        if Plr == LocalPlayer and ChatChannel then
            ChatChannel:SendAsync(SECRET_CHAT_CODE)
        end
    end)
    
    Players.PlayerRemoving:Connect(function(Plr)
        if getgenv().ActiveClients[Plr.UserId] then
            getgenv().ActiveClients[Plr.UserId] = nil
            local playerData = activeClientsFolder:FindFirstChild(tostring(Plr.UserId))
            if playerData then
                playerData:Destroy()
            end
    ...
    activeClientsFolder.ChildAdded:Connect(function(child)
        if child:IsA("StringValue") and child.Name ~= tostring(LocalPlayer.UserId) then
            task.wait(0.5)
            if child and ChatChannel then
                ChatChannel:SendAsync(SECRET_CHAT_CODE)
            end
        end
    end)
    ...
    if getgenv().already_loaded_mute_keybind then
        ...
        elseif not getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput") then
            local function toggle_mute()
                ...
                getgenv().VoiceChatInternal:SubscribePause(getgenv().LocalPlayer.UserId, not getgenv().VoiceChatInternal:IsSubscribePaused(getgenv().LocalPlayerUserId))

                local vc_result_paused = getgenv().VoiceChatInternal:IsSubscribePaused(getgenv().LocalPlayer.UserId)

                if vc_result_paused == true then
                    getgenv().notify("Success", "Muted Microphone.", 6)
                elseif vc_result_paused == false then
                    getgenv().notify("Success", "Unmuted Microphone.", 6.5)
                end
            end
    ...
    if getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput") then
        ...
        Section21 = Tab21:CreateSection("||| Voice-Chat Section |||")
    else
        Tab21 = Window:CreateTab("> VoiceChat", getgenv().image_use_zacks)
        Section21 = Tab21:CreateSection("||| VC Configuration Section |||")
    end
    wait()
    ...
    wait()
    if result_get == false then
        Tab19 = Window:CreateTab("> Visuals", getgenv().image_use_zacks)
        Section19 = Tab19:CreateSection("||| Visuals/ESP Section |||")
    elseif result_get == true then
        Tab19 = Window:CreateTab("> Drawing", getgenv().image_use_zacks)
        Section19 = Tab19:CreateSection("||| Drawing/ESP Section |||")
    else
        Tab19 = Window:CreateTab("> ESP", getgenv().image_use_zacks)
        Section19 = Tab19:CreateSection("||| ESP's Section |||")
    end
    wait()
    ...
    Section15 = Tab15:CreateSection("||| Keybinds and Settings Section |||")
    ...
    getgenv().LocalPlayer.OnTeleport:Connect(function(State)
        if (not getgenv().TeleportCheck) and getgenv().queueteleport then
            getgenv().TeleportCheck = true
            queueteleport("loadstring(game:HttpGet(('https://raw.githubusercontent.com/EnterpriseExperience/MicUpSource/refs/heads/main/BETA-Loading')))()")
        end
    end)
    wait(0.2)
    ...
    Callback = function(selectedEmote)
        getgenv().Humanoid:PlayEmote(selectedEmote)
        getgenv().HumanoidRootPart.Anchored = true
        wait(0.2)
    ...
    Callback = function(moonCrazy)
        ...
        if moonCrazy then
            ...
            if not getgenv().SpookyMoon or getgenv().SpookyMoon == false then
                resetLightingSettings()
                wait(0.3)
                sky.MoonAngularSize = 11
                wait(0.3)
    ...
    elseif not getgenv().Character.Animate:FindFirstChild("walk") then
        getgenv().Easies_Configuration["Emote_Keybinds"] = "off"
        getgenv().notify("Sorry!", "Custom Animation Packages cannot be loaded here.", 6)
    else
        warn("Not enabled in Configuration.")
    end
    wait()
    ...
    Callback = function(KeyForTripping)
        local newKey = convertToKeyCode(KeyForTripping)
        if newKey then
    ...
    Callback = function(setKeyForFakeOut)
        local newKey = convertToKeyCode(setKeyForFakeOut)
        if newKey then
    ...
    Callback = function(setDefaultFlagForCustomAnims)
        if setDefaultFlagForCustomAnims then
            getgenv().using_custom_animation_packages = true
            Zacks_Easy_Configuration["Custom_Animation_Package_System"] = "on"
        else
            getgenv().using_custom_animation_packages = false
            Zacks_Easy_Configuration["Custom_Animation_Package_System"] = "off"
        end
    end,})

    getgenv().IdleAnimToSet = Tab20:CreateDropdown({
    Name = "Idle Animation",
    Options = {"Zombie", "Catwalk Glam", "Elder", "Cartoony", "Adidas", "Werewolf", "Vampire", "Astronaut", "Superhero", "Knight", "Mage", "Ninja", "Toy", "NFL", "No Boundaries", "Oldschool", "Pirate", "Levitation", "Bubbly", "Robot", "Wicked Popular", "Bold", "Styli1sh", "Rthro"},
    ...
    Callback = function(deathOnFullLoad)
        if deathOnFullLoad then
            getgenv().die_upon_loading = true
            Zacks_Easy_Configuration.Death_On_Load = "on"
        else
            getgenv().die_upon_loading = false
            Zacks_Easy_Configuration.Death_On_Load = "off"
        end
    end,})
    ...
    Callback = function(ezEmoteBindings)
        if ezEmoteBindings then
            getgenv().emoting_bindings = true
            Zacks_Easy_Configuration.Emote_Keybinds = "on"
        else
            getgenv().emoting_bindings = false
            Zacks_Easy_Configuration.Emote_Keybinds = "off"
        end
    end,})
    ...
    Callback = function(antiAfkScript)
        if antiAfkScript then
            getgenv().anti_afk_toggle = true
            Zacks_Easy_Configuration.AntiAFK = "on"
        else
            getgenv().anti_afk_toggle = false
            Zacks_Easy_Configuration.AntiAFK = "off"
        end
    end,})
    ...
    Callback = function(loadscreen)
        if loadscreen then
            getgenv().Load_Screen = true
            Zacks_Easy_Configuration.Loading_Screen = "on"
        else
            getgenv().Load_Screen = false
            Zacks_Easy_Configuration.Loading_Screen = "off"
        end
    end,})
    ...
    Callback = function(toggleTheBoomboxSound)
        if toggleTheBoomboxSound then
            getgenv().boomboxes_on = true
            Zacks_Easy_Configuration.Mute_Boomboxes_Cellmates_VC_Game_Setting = "on"
        else
            getgenv().boomboxes_on = false
            Zacks_Easy_Configuration.Mute_Boomboxes_Cellmates_VC_Game_Setting = "off"
        end
    end,})
    ...
    Callback = function(toggleTheMusicSound)
        if toggleTheMusicSound then
            getgenv().music_mute_configured = true
            Zacks_Easy_Configuration.Mute_Music_Volume_Cellmates_VC_Game_Setting = "on"
        else
            getgenv().music_mute_configured = false
            Zacks_Easy_Configuration.Mute_Music_Volume_Cellmates_VC_Game_Setting = "off"
        end
    end,})
    ...
    Callback = function(toggleTheSFXSounds)
        if toggleTheSFXSound then
            getgenv().sfx_sounds_on = true
            Zacks_Easy_Configuration.Mute_Sound_Effects_Cellmates_VC_Game_Setting = "on"
        else
            getgenv().sfx_sounds_on = false
            Zacks_Easy_Configuration.Mute_Sound_Effects_Cellmates_VC_Game_Setting = "off"
        end
    end,})
    ...
    Callback = function(stringForSysBroken)
        if stringForSysBroken then
            getgenv().sys_broken_label = true
            Zacks_Easy_Configuration.System_Broken_Title = "on"
        else
            getgenv().sys_broken_label = false
            Zacks_Easy_Configuration.System_Broken_Title = "off"
        end
    end,})
    wait(0.2)
    if getgenv().sys_broken_label or getgenv().sys_broken_label == true then
        ...
        warn("System Broken Title is off.")
    end
    ...
    Callback = function(turnOnFullyLoadedMsg)
        if turnOnFullyLoadedMsg then
            getgenv().IsFullLoadedMessage = true
            Zacks_Easy_Configuration.Fully_Loaded_Messaging = "on"
        else
            getgenv().IsFullLoadedMessage = false
            Zacks_Easy_Configuration.Fully_Loaded_Messaging = "off"
        end
    end,})
    wait(0.2)
    if getgenv().IsFullLoadedMessage or getgenv().IsFullLoadedMessage == true then
        ...
        warn("Fully Loaded Message is not on.")
    end
    ...
    Callback = function(bigBasePlate)
        if bigBasePlate then
            getgenv().biggest_baseplate_yes = true
            Zacks_Easy_Configuration.Huge_Baseplate = "on"
        else
            getgenv().biggest_baseplate_yes = false
            Zacks_Easy_Configuration.Huge_Baseplate = "off"
        end
    end,})
    ...
    Callback = function(clockTimeMenu)
        if clockTimeMenu then
            getgenv().day_night_cycle_script_gui = true
            Zacks_Easy_Configuration.Script_Clock_Time_GUI = "on"
        else
            getgenv().day_night_cycle_script_gui = false
            Zacks_Easy_Configuration.Script_Clock_Time_GUI = "off"
        end
    end,})
    ...
    Callback = function(vcIsNotSuspended)
        if vcIsNotSuspended then
            getgenv().haha_never_suspend = true
            Zacks_Easy_Configuration.Anti_Suspend_VC = "on"
        else
            getgenv().haha_never_suspend = false
            Zacks_Easy_Configuration.Anti_Suspend_VC = "off"
        end
    end,})
    ...
    Callback = function(IsInfYieldPrem)
        if IsInfYieldPrem then
            getgenv().new_modified_inf_yield = true
            Zacks_Easy_Configuration.Infinite_Yield_Premium = "on"
        else
            getgenv().new_modified_inf_yield = false
            Zacks_Easy_Configuration.Infinite_Yield_Premium = "off"
        end
    end,})
    ...
    Callback = function(getPerformanceGUI)
        if getPerformanceGUI then
            getgenv().performance_is_toggled = true
            Zacks_Easy_Configuration.Performance_Statistics = "on"
        else
            getgenv().performance_is_toggled = false
            Zacks_Easy_Configuration.Performance_Statistics = "off"
        end
    end,})
    ...
    Callback = function(notNewMaterials)
        if notNewMaterials then
            getgenv().utilizing_old_stuff_materials = true
            Zacks_Easy_Configuration.Old_Materials = "on"
        else
            getgenv().utilizing_old_stuff_materials = false
            Zacks_Easy_Configuration.Old_Materials = "off"
        end
    end,})
    ...
    Callback = function(modifiedSysBrokenLoading)
        if modifiedSysBrokenLoading then
            getgenv().usingOldSysBroken = true
            Zacks_Easy_Configuration.Old_Materials = "on"
        else
            getgenv().usingOldSysBroken = false
            Zacks_Easy_Configuration.Old_Materials = "off"
        end
    end,})
    ...
    Callback = function(changeTPToolRespawn)
        if changeTPToolRespawn then
            getgenv().modifiedTPToolBruh = true
            Zacks_Easy_Configuration.keep_tp_tool = "on"
        else
            getgenv().modifiedTPToolBruh = false
            Zacks_Easy_Configuration.keep_tp_tool = "off"
        end
    end,})
    wait(0.2)
    ...
    local function saveConfig()
        ...
        getgenv().notify("Success!", "Configuration has been saved successfully.", 6)
    end
    ...
    getgenv().saveYourConfigurationZEH = Tab20:CreateButton({
    Name = "Save Zacks Easy Hub Configuration",
    Callback = function()
        saveConfig()
    end,})
    ...
    GuiService:SendNotification({
        ...
        Text = "[TASK_SCHEDULER_PROCESS_0xFFFFFFF]- MEMORY ACCESS_CODE",
    })
    wait(0.3)
    GuiService:SendNotification({
        Title = "Enjoy!",
        Text = "Watching Background Processes (Crashes, Reports, etc).",
    })
