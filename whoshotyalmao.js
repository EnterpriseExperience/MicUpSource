    ...
    game.Players.LocalPlayer.CharacterAdded:Connect(function(newChar)
        wait(1)
        if not newChar then return warn("Character did not load properly! or Character did not load in time!") end
        local player = game.Players.LocalPlayer
        
        repeat task.wait(3) until newChar:FindFirstChild("Humanoid") and newChar:FindFirstChild("HumanoidRootPart")
        
        if not newChar:FindFirstChild("ZacksEasyBillboard") then
            ChatChannel:SendAsync(SECRET_CHAT_CODE)
            wait(0.1)
            createTitle(player)
        end
