...
wait()
getgenv().KeepConsoleOpen = true
getgenv()._ConsoleLoop = getgenv()._ConsoleLoop or nil
...
if getgenv()._ConsoleLoop then
    task.cancel(getgenv()._ConsoleLoop)
end
wait()
getgenv()._ConsoleLoop = task.spawn(function()
    while getgenv().KeepConsoleOpen do
        openDevConsole()
        task.wait(0.2)
    end
end)
...
function copy_plr_avatar(Player)
   ...
   local function copyAvatar(target)
      ...
      if height then
         pcall(function() getgenv().Send("body_scale", "HeightScale", height * 100) end)
      end
      if width then
         pcall(function() getgenv().Send("body_scale", "WidthScale", width * 100) end)
      end
