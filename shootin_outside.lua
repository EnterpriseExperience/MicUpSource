...
--]]
    ...
    Callback = function(antiVoidFall)
        if antiVoidFall then
            ...
            local function creating_parts_process()
                local baseplate = Instance.new("Part")
                baseplate.Name = "ANTI_VOID_BASEPLATE"
                baseplate.Size = Vector3.new(60, 1, 60)
                baseplate.Anchored = true
                baseplate.CanCollide = true
                baseplate.Material = Enum.Material.Air
                baseplate.Transparency = 0.3
                baseplate.Color = Color3.fromRGB(123, 0, 123)
                baseplate.Parent = Workspace
                getgenv().BasePlate_Saved = baseplate
                return baseplate
            end
            
            local function update_parts_correctly()
                local baseplate = creating_parts_process()
                while getgenv().anti_void_player == true do
                    local character = return_correct_char()
                    local rootPart = character:FindFirstChild("HumanoidRootPart")
                    if rootPart then
                        local newPos = rootPart.Position - Vector3.new(0, rootPart.Size.Y / 2 + baseplate.Size.Y / 2, 0)
            ...
            update_parts_correctly()
        else
            getgenv().anti_void_player = false
            getgenv().AntiVoidPlayer:Set(false)
            if getgenv().BasePlate_Saved or game:GetService("Workspace"):FindFirstChild("ANTI_VOID_BASEPLATE") then
                local Anti_Voided_Part = game:GetService("Workspace"):FindFirstChild("ANTI_VOID_BASEPLATE")

                if Anti_Voided_Part then
                    Anti_Voided_Part:Destroy()
                    wait(0.2)
                    if getgenv().BasePlate_Saved then
                        getgenv().BasePlate_Saved = nil
                    end
                end
            end
        end
