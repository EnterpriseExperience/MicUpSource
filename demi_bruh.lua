...
--]]
    ...
    Callback = function(Bind_Freeze)
        if not Bind_Freeze then return warn("Invalid keybind selection:", Bind_Freeze) end
        wait()
    ...
    Callback = function(Bind_Reverse)
        if not Bind_Reverse then return warn("Invalid keybind selection:", Bind_Reverse) end
        wait()
