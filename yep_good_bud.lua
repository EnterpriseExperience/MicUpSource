...
--]]
    ...
    Callback = function(slot)
        ...
        if typeof(slot) ~= "EnumItem" then
            return warn("Invalid slot selection: Expected Enum.KeyCode, got: "..typeof(slot))
        end

        selectedSlot = tostring(slot)

        getgenv().notify("Success:", "Selected Slot: "..selectedSlot, 6)
    end,})
