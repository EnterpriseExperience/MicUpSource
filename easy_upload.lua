...
function change_idle_anim(input, input_2)
    local player = game.Players.LocalPlayer
    local character = player.Character or player.CharacterAdded:Wait()
    local Animate = character:WaitForChild("Animate", 0.4)
    local humanoid = character and character:FindFirstChildWhichIsA("Humanoid") or character:WaitForChild("Humanoid", 1)
...
if getgenv().scriptEnabled then
    ...
    if getgenv().Easies_Configuration["Emote_Keybinds"] == true then
        ...
        local Players = cloneref and cloneref(game:GetService("Players")) or game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
        local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait(0.5)
        local character = Character or LocalPlayer.CharacterAdded:Wait(1)
        local Humanoid = Character and Character:FindFirstChildWhichIsA("Humanoid") or Character:WaitForChild("Humanoid", 2)
        local humanoid = Humanoid or character:WaitForChild("Humanoid", 5)
        local human = Humanoid or character:WaitForChild("Humanoid", 7)
        local UserInputService = game:GetService("UserInputService")
