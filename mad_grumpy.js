...
--]]
    ...
    wait()
    local Emote_Settings = {
        ["SpeedUp_Speed"] = 4,
        ["SlowDown_Speed"] = 0.1,
        ["Freeze_Keybind"] = "V",
        ["Reverse_Keybind"] = "F",
        ["NormalSpeed_Keybind"] = "X",
        ["SlowDown_Keybind"] = "Q",
        ["SpeedUp_Keybind"] = "E"
    }
    ...
    getgenv().PickASlot = Tab2:CreateDropdown({
    ...
    Callback = function(slot)
        if typeof(slot) ~= "string" then
            return warn("Invalid slot selection: Expected string, got: "..typeof(slot))
        end

        selectedSlot = Slots_Table[slot]

        if not selectedSlot then
            return warn("Error: No matching KeyCode found for selection.")
        end

        getgenv().notify("Success:", "Selected Slot: "..slot.." (KeyCode: "..tostring(selectedSlot)..")", 6)
    end,})

    getgenv().SelectAnEmote = Tab2:CreateDropdown({
    ...
    Callback = function(emote)
        ...
        getgenv().notify("Success:", "Assigned emote: '"..emote.."' to slot: "..selectedSlot.Name, 6)
    end,})

    getgenv().EmoteSpeedSlider = Tab2:CreateSlider({
    Name = "Emote Configuration Speed Slider",
    Range = {0, 50},
    Increment = 0.1,
    Suffix = "",
    CurrentValue = 1,
    Flag = "SlidingTheEmoteSpeedEz",
    Callback = function(Get_Emote_Speed)
        getgenv().emoting_actions(Get_Emote_Speed)
    end,})
    wait()
    ...
    wait(0.5)
    local GuiService = cloneref and cloneref(game:GetService("CoreGui")) or game:GetService("GuiService")
