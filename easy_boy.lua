...
--]]
    ...
    function findJumpAnimation()
        ...
        local LocalPlayer = Players.LocalPlayer
        local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
        local animateScript = Character and Character:WaitForChild("Animate", 3)
        ...
        if jumpAnim and jumpAnim:IsA("Animation") then
            print(tostring(jumpAnim).." was found!")
            return jumpAnim
        else
    ...
    function findWalkAnimation()
        ...
        if walkAnim and walkAnim:IsA("Animation") then
            print(tostring(walkAnim).." was found!")
            return walkAnim
        else
            return warn("Jump Animation not found inside the 'jump' object.")
        end
    ...
    function findRunAnimation()
        ...
        if runAnim and runAnim:IsA("Animation") then
            print(tostring(runAnim).." was found!")
            return runAnim
        else
    ...
    function findFallAnimation()
        ...
        if fallAnim and fallAnim:IsA("Animation") then
            print(tostring(fallAnim).." was found!")
            return fallAnim
        else
    ...
    function findClimbAnimation()
        ...
        if ClimbAnim and ClimbAnim:IsA("Animation") then
            print(tostring(ClimbAnim).." was found!")
            return ClimbAnim
        else
    ...
    function findIdleAnimations()
        ...
        local animateScript = character and character:WaitForChild("Animate", 2)
        ...
        if idleAnim1 and idleAnim1:IsA("Animation") and idleAnim2 and idleAnim2:IsA("Animation") then
            print(tostring(idleAnim1).." and "..tostring(idleAnim2).." we're found!")
    ...
    function change_idle_anim(input, input_2)
        ...
        if not Animate then
            return warn("Animate LocalScript was not found within Character.")
        end
        wait(0.1)
        Animate.Disabled = true
        task.wait()
    ...
    function change_walk_anim(input)
        ...
        local humanoid = character and character:FindFirstChildWhichIsA("Humanoid") or character:WaitForChild("Humanoid", 2)
        if players and character and humanoid or character:FindFirstChildWhichIsA("Humanoid") then
            for _, track in pairs(humanoid:GetPlayingAnimationTracks()) do
                track:Stop()
            end
        end
    ...
    --[[if getgenv().loaded_animation_system then
        ...
        if getgenv().Easies_Configuration["Default_Animation_Package_System"] == true then
            ...
            warn("[Animation_Packages_System]: Not enabled in Configuration.")
        end
    end--]]
    wait(0.2)
    function r15(plr)
    ...
    if getgenv().Easies_Configuration["Default_Animation_Package_System"] == true then
        local animationMappings = {
            ...
            Rthro = {Idle = {Rthro_Idle_1, Rthro_Idle_2}, Walk = Rthro_Walk, Run = Rthro_Run, Fall = Rthro_Fall, Jump = Rthro_Jump, Climb = Rthro_Climb}
        }

        function connect_to_animations()
            for packageName, anims in pairs(animationMappings) do
                if getgenv().Easies_Configuration["Animation_Package_Idle"] == packageName then
                    change_idle_anim(unpack(anims.Idle))
                end
                if getgenv().Easies_Configuration["Animation_Package_Walk"] == packageName then
                    change_walk_anim(anims.Walk)
                end
                if getgenv().Easies_Configuration["Animation_Package_Run"] == packageName then
                    change_run_anim(anims.Run)
                end
                if getgenv().Easies_Configuration["Animation_Package_Fall"] == packageName then
                    change_fall_anim(anims.Fall)
                end
                if getgenv().Easies_Configuration["Animation_Package_Jump"] == packageName then
                    change_jump_anim(anims.Jump)
                end
                if getgenv().Easies_Configuration["Animation_Package_Climb"] == packageName then
                    change_climb_anim(anims.Climb)
                end
            end
        end
        wait(0.2)
        function connect_anims()
            local player = game.Players.LocalPlayer

            player.CharacterAdded:Connect(connect_to_animations)
    ...
    if getgenv().scriptEnabled then
        ...
        if getgenv().Easies_Configuration["Emote_Keybinds"] == true then
            ...
            local LocalPlayer = Players.LocalPlayer
            local Character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
            local character = Character or LocalPlayer.CharacterAdded:Wait()
            local Humanoid = Character and Character:WaitForChild("Humanoid", 5)
            local humanoid = Character and character:WaitForChild("Humanoid", 5)
            ...
            wait(0.2)
            repeat wait(5) until character and Humanoid and character:FindFirstChild("Humanoid") and character:FindFirstChild("HumanoidRootPart")
            wait()
            ...
            local function playEmote(emoteId)
                if not humanoid then
                    ...
                    repeat wait(5) until character and humanoid and humanoid:IsDescendantOf(game)
                    wait(2)
                    if character and humanoid then
                        print("Found Character and Humanoid, Continuing.")
                    else
                        warn("Humanoid was not allocated during initialization.")
                    end
                end
                repeat wait(3) until character and humanoid and humanoid:IsDescendantOf(game)
                if character and humanoid and humanoid:IsDescendantOf(game) then
                    for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
                        track:Stop()
                    end
                    wait()
