...
wait()
getgenv().FreeEmotes_GUI = ScreenGui
...
getgenv().search_modes = getgenv().search_modes or search_modes
	...
	for _, sortType in ipairs(getgenv().search_modes) do
		...
		while true do
			...
			for _, emote in ipairs(page) do
				if not collected[emote.Id] then
					collected[emote.Id] = emote
				end
			end
